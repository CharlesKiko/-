...
import { ElMessage, ElTable, type FormInstance, type ElUpload, type UploadFile } from 'element-plus'
...
export default function () {
    ...
    function beforeUploadVideo(file: any) {
        ...
        if (!isLt100M) {
            ElMessage.error('上传视频大小不能超过50MB哦!')
            return false
        }
    }
    ...
    function delCourse() {
        ...
        axios.post('http://localhost:8080/course/del', ids, {
            headers: {
    ...
    function search() {
        ...
        axios.post('http://localhost:8080/course/search', searchCourse.value).then(res => {
            if (res.data.isOk) {
                ElMessage.success(res.data.msg);
                courses.value = res.data.courses;
            } else {
    ...
    function publish() {
        ...
        if (statues.includes("通过")) {
            ElMessage.error('选中课程包括已通过审核的课程，请重新选择');
            return;
        }
        ...
        axios.post('http://localhost:8080/course/publish', coursess, {
            headers: {
                'Content-Type': 'application/json'
            }
        }).then(res => {
